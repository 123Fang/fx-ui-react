    ...
    if (duration as number > 0) {
      setTimeout(() => {
        ...
        setTimeout(() => {
          ...
          container?.removeChild(document.querySelector('.fx_loading_container') as Node);
        }, 300)
      }, duration as number)
    }
  }, [open])
    ...
    >
      <div className={loadingClass} style={style || loadingStyle}>
        <div className="container_inner">
          {
            icon?.includes('fx-icon-') ? <i className={[icon, 'fx_publicRotateEle'].join(' ')} /> : <img src={icon} alt="加载失败" className="fx_publicRotateEle" />
          }
          {
