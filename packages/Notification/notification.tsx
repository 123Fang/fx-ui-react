...
el.className = 'fx_notification_container';
document.body.appendChild(el);
wrapper.className = 'fx_notification_wrapper';
...
function NotificationItem(props: NotificationItemProps): JSX.Element {
  ...
  const icons = {
    info: 'fx-icon-prompt-filling',
    success: 'fx-icon-success-filling',
    warning: 'fx-icon-warning-filling',
    error: 'fx-icon-delete-filling',
  };
  ...
  useEffect(() => {
    ...
    if (time) {
      ...
      timer = setTimeout(() => {
        const ele = document.querySelector('.fx_notification');
        if (ele) {
          document.querySelector('.fx_notification_wrapper')?.removeChild(ele);
        }
        setIsOpen(false);
      }, time * 1000);
    }
        ...
        >
          <div
            className={classNames(
              'fx_demo_notification',
              `fx_demo_notification_${getPlacement()}`,
              `${className}`
            )}
            style={{
              top: getPosition('top'),
          ...
          >
            {icon && <span className="fx_notification_icon">{icon}</span>}
            {!icon && type && (
              <span className="fx_notification_icon">
                <i className={classNames(`${icons[type]}`)} />
              </span>
            )}
            <div
              className="fx_notification_content"
              onClick={onClickNotification}
            >
              <div className="fx_notification_title_wrapper">
                <span className="fx_notification_title">{message}</span>
                <span
                  onClick={(e) => onCloseNotification(e)}
                  className="fx_notification_close"
                >
                  {closeIcon || globalParams.closeIcon || (
                    <i className="fx-icon-close" />
                  )}
                </span>
              </div>
              <div className="fx_notification_description">{description}</div>
              {btn && <div className="fx_notification_operation">{btn}</div>}
            </div>
          </div>
        </CSSTransition>
      )}
...
function popNotification(config: NotificationItemProps, type: TypeProps) {
  ...
  root.className = 'fx_notification';
  ReactDOM.createRoot(root).render(
    <NotificationItem {...config} type={type} />
  );
  const wrapperEl = document.querySelector('.fx_notification_wrapper');
  if (wrapperEl) {
